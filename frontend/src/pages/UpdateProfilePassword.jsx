...
function UpdateProfilePassword() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const res = await axios.post(`${apiUrl}/update_profile_password.php`, { ...formData, id: JSON.parse(localStorage.getItem("user")).id });
  ...
  const profileSchema = Yup.object({
    ...
    newPassword: Yup.string().required("New Password is Required").min(8, ' Password must be at least 8 characters')
      ...
      .matches(/[0-9]/, 'Must contain at least one number')
      .matches(/[@$!%*?&]/, 'Must contain at least one special character'),
    confirmPassword: Yup.string().required("Confirm Password is Required").oneOf([Yup.ref('newPassword'), null], 'Passwords must match').required('Confirm Password is required'),
  });
  return (
      ...
      <div className="container p-5" data-aos="fade-up" data-aos-delay={100}>
        ...
        >
          ...
          <div className="col-lg-5 mx-auto card p-4">
            <h3 className='text-center form-title fw-bold pb-3'>Change Profile Password </h3>
            <Formik
              initialValues={{
                currentPassword: '',
                newPassword: '',
              ...
              onSubmit={async (values, { setSubmitting }) => {
                ...
                try {
                  ...
                  const res = await axios.put(`${apiUrl}/users/change-profile-password`, { ...values, userId: JSON.parse(localStorage.getItem("user"))._id });
                  // console.log(res.data);
                  toast.success('Password changed successfully!');
                  navigate("/");
                } catch (err) {
            ...
            <div className="col-md-12">
              ...
              />
                    ...
                    <div className="col-md-12 text-center">
                      ...
                      <button type="submit" >Change Profile Password</button>
                    </div>
                  </div>
                </form>)}
            </Formik>
